...
export default function IntelligenceForm() {
  ...
  const [categoryStr, setCategoryStr] = useState("tech-intelligence");
  const [customImageUrl, setCustomImageUrl] = useState("");
  const [previewSeed, setPreviewSeed] = useState<number | null>(null);

  // Generate Pollinations URL on the fly
  const generatedImageUrl = previewSeed !== null && titleStr.trim() !== "" 
    ? (() => {
        const categoryModifier = categoryStr === "esports-intelligence" 
          ? "esports gaming tournament" 
          : categoryStr.replace("-", " ");
        const imagePrompt = `highly detailed cinematic futuristic intelligence report cover art about ${titleStr}. ${categoryModifier} theme, Cyberpunk data visualization, neon green accents on dark background, masterpiece, 8k resolution, photorealistic, clean composition`;
  ...
  const handleSubmit = async (formData: FormData) => {
    // Inject content, status, and the exact previewed image URL so the server doesn't roll a new seed
  ...
  return (
    <form action={handleSubmit} className="max-w-3xl mx-auto space-y-8 pb-32">
      <div className="flex justify-between items-center bg-dark-pitch p-4 rounded-xl border border-white/5">
        <h2 className="text-xl font-mono text-eagles-green font-black uppercase tracking-wider">New Intelligence Brief</h2>
      ...
      <div className="flex gap-4 text-sm text-muted-foreground/60 flex-wrap">
        <select
          ...
          className="bg-[#0a0a0a] border border-white/10 rounded px-3 py-2 text-white font-mono text-xs uppercase focus:border-eagles-green outline-none"
        >
      ...
      <div className="space-y-4 p-5 rounded-xl border border-white/10 bg-[#0d0d0d]">
        ...
        {generatedImageUrl && !customImageUrl && (
          <div className="relative w-full aspect-[1.9] bg-black rounded-lg overflow-hidden border border-white/5">
            ...
            <Image 
              ...
              unoptimized // Standard next/image behavior for external dynamic URLs
            />
          </div>
        )}
        ...
        <div className="pt-2">
          ...
          <input
            ...
            className="w-full p-3 bg-white/[0.02] border border-white/10 rounded-lg text-sm font-mono placeholder:text-muted-foreground/50 focus:border-eagles-green transition-colors outline-none"
      ...
      <div className="space-y-4">
        <input
          name="sourceUrl"
          type="url"
          className="w-full p-3 bg-surface/50 border border-border rounded-lg text-sm font-mono placeholder:text-muted-foreground/50 focus:border-eagles-green transition-colors"
      ...
      <div className="min-h-[50vh] border border-border rounded-xl bg-surface/30 overflow-hidden">
        <div className="p-3 bg-secondary/30 border-b border-border font-mono text-xs text-muted-foreground flex items-center gap-2">
          ...
          <span className="ml-2 uppercase tracking-widest text-[9px] text-eagles-green">Classified Dossier Editor</span>
        </div>
        <div className="p-4">
